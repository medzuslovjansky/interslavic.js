import {BareRecord, SimpleSynset} from "@interslavic/steen-utils";
...
export class Multirator {
  ...
  public process(record: BareRecord): ProcessedLemma[] {
    ...
    let currentReplacements = new Set<string>();
    for (const option of record.isv.options) {
      currentReplacements.add(option.value);
      triggers.set(option.value, []);
    }
    ...
    for (const rule of this.rules) {
      ...
      for (const sourceString of currentReplacements) {
        for (const replacement of rule.replaceWith) {
          const processedString = rule.id.indexOf('*') !== 0
            // @ts-ignore
            ? sourceString.replace(rule.match, replacement)
            : sourceString.replace(IS_LETTER, (word) => {
                // @ts-ignore
                return word.replace(rule.match, replacement);
              });

          nextReplacements.add(processedString);
    ...
    for (const value of currentReplacements) {
      result.push({
        value,
  ...
  public getDifference(flavorizedLemmas: ProcessedLemma[], targetSynset?: SimpleSynset | null): number {
    let minimalDistance = Number.POSITIVE_INFINITY;

    const targetOptions = targetSynset ? targetSynset.options : [];
    for (const flavor of flavorizedLemmas) {
      const flavorValue = flavor.value.toLowerCase();
      for (const option of targetOptions) {
        const optionValue = option.value.toLowerCase();
        const avgLength = 0.5 * (flavorValue.length + option.value.length);
        const distance = getEditingDistance(flavorValue, optionValue);
        const relativeDistance = distance / avgLength;
        minimalDistance = Math.min(relativeDistance, minimalDistance);
      }
    }
